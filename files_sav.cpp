...
void open_captfile(LPCTSTR lpFName)
{
    ...
    if (CAPTFILE.filehandle!=INVALID_HANDLE_VALUE)
	{
		...
		if (strstr(header.description,"BrainBay Archive File")!=NULL)
		{
			...
			TTY.devicetype=DEV_MODEEG_P2;
			for (i=0;i<20;i++)
			  if (!strcmp(header.devicetype, devicetypes[DEV_MODEEG_P3])) TTY.devicetype=i;
		...
		GLOBAL.minimized=FALSE;
		 ...
		 if (try_connect) {
			 if (TTY.devicetype==DEV_NIA) {
					// DisconnectNIA();								
					TTY.CONNECTED = ConnectNIA(NULL);
			 }
			 else { TTY.CONNECTED=SetupCommPort(TTY.PORT); update_p21state(); } 
		 }
